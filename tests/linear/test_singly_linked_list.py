...
def test_extend_and_from_iterable():
    ...
    lst.extend([2, 3])
    assert lst.to_list() == [1, 2, 3]
    lst2 = SinglyLinkedList.from_iterable([4, 5])
...
def test_contains_operator():
    lst = SinglyLinkedList([1, "abc", None])
    assert 1 in lst
    assert "abc" in lst
    assert None in lst
...
def test_str_and_repr():
    ...
    s = str(lst)
    r = repr(lst)
    assert s.startswith("SinglyLinkedList")
    assert r.startswith("SinglyLinkedList")
...
def test_non_numeric_types():
    class Dummy: pass
    obj = Dummy()
    lst = SinglyLinkedList(["a", obj, 3.14])
    assert lst[1] is obj
    assert "a" in lst
    assert 3.14 in lst
...
def test_bulk_operations_on_empty():
    ...
    lst.extend([])
    assert lst.to_list() == []
    lst2 = SinglyLinkedList.from_iterable([])
    assert lst2.to_list() == []